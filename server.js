...
function seededShuffle(items, seed) {
  return [...items]
    .map((item) => ({
      item,
      sortKey: crypto.createHash("sha256").update(`${seed}|${productKey(item)}|${item.category || ""}`).digest("hex"),
    }))
...
async function buildCatalogPool() {
  ...
  for (let i = 0; i < seeds.length; i += 1) {
    ...
    try {
      ...
      for (const entry of scored) {
        ...
        const key = String(entry.candidate?.id ?? `${entry.candidate?.ean || "no-ean"}|${entry.candidate?.store?.code || ""}|${entry.candidate?.name || ""}`);
      ...
      report.perSeed.push({ searchTerm: seed.searchTerm, group: seed.group, category: seed.category, added });
    } catch (error) {
      report.errors.push({ searchTerm: seed.searchTerm, error: error.message });
    }
  ...
  await writeJson(path.join(DATA_DIR, "products.json"), items);
  await writeJson(path.join(DATA_DIR, "catalog-build-report.json"), report);
  return pool;
}
...
async function loadPoolItems() {
  ...
  const products = await readJson(path.join(DATA_DIR, "products.json"), []);
...
function pruneHistory(entries, date) {
  return (entries || []).filter((entry) => entry && typeof entry.date === "string" && dayDiff(entry.date, date) < HISTORY_RETENTION_DAYS);
}
...
function historyExcludedKeySet(history, date, cooldownDays, currentItems = []) {
  ...
  for (const entry of history.entries || []) {
    ...
    for (const key of entry.keys || []) keys.add(String(key));
  }
  return keys;
}
...
function buildDailyCatalogFromPool(poolItems, date, excludedKeys = new Set()) {
  ...
  for (const product of filtered) {
    const group = GROUP_MAP[product.group || "middag"] || product.group || "middag";
...
async function buildFreshDailyCatalog(date, currentCatalog = {}) {
  ...
  if (!poolItems.length) {
    return { ok: false, error: "Kunne ikke laste katalogpoolen.", poolCount: 0 };
  }
  ...
  return { ok: false, error: "Fant ikke nok ferske varer i katalogpoolen.", poolCount: poolItems.length };
}
...
function clientIpHash(req) {
  return crypto.createHash("sha256").update(`${ROOT_DIR}|ip|${clientIp(req)}`).digest("hex");
}
...
function buildSessionRounds(products, sessionId, roundCount) {
  ...
  for (let i = 0; i < roundCount; i += 1) {
    ...
    const b = pool[i * 2 + 1];
    if (!a || !b) break;
    const optionPrices = [Math.round(Number(a.price)), Math.round(Number(b.price))];
    const targetIndex = parseInt(crypto.createHash("sha256").update(`${sessionId}|target|${i}`).digest("hex").slice(0, 2), 16) % 2;
    rounds.push({
      optionKeys: [productKey(a), productKey(b)],
      optionPrices,
...
async function buildSessionPayload(catalog, req, clientId = "") {
  ...
  if (rounds.length < roundTarget()) {
    ...
    if (merged.length > products.length) {
      products = merged;
      rounds = buildSessionRounds(products, sessionId, maxRounds());
    }
  }
  ...
  if (products !== (catalog.items || [])) {
    payload.items = products;
    payload.itemCount = products.length;
  }
  return payload;
}

function normalizeScoreDay(day, date) {
  const entriesByClientId = (day && typeof day === "object" && day.entriesByClientId && typeof day.entriesByClientId === "object")
...
function validateSubmitPayload(body) {
  const { sessionId = "", sessionToken = "", clientId = "", score, correctCount, answeredCount } = body || {};
...
function requireValidSession(sessions, req, sessionId, sessionToken, clientId) {
  ...
  if (tokenHash !== session.tokenHash) throw Object.assign(new Error("Ugyldig sesjonstoken."), { status: 403 });
  if (session.clientId) {
    if (clientId !== session.clientId) throw Object.assign(new Error("Spillsesjonen må fullføres fra samme nettleser."), { status: 403 });
  ...
  if (session.submittedAt) throw Object.assign(new Error("Denne spillsesjonen er allerede sendt inn."), { status: 409 });
  return session;
}
...
async function handleDiagnostics(req, res) {
  ...
  if (req.query.probe === "1") {
    try {
      ...
      payload.groceryProbe = { ok: true, itemCount: results.length };
    } catch (error) {
      payload.groceryProbe = { ok: false, error: error.message };
    }
  }
  res.json(payload);
}
...
app.get("/api/diagnostics", handleDiagnostics);

async function handleGrocerySearch(req, res, next) {
  try {
    ...
    res.json({ ok: true, query: q, itemCount: results.length, data: results });
  } catch (error) {
    next(error);
  }
}

app.get("/grocery/search", handleGrocerySearch);
app.get("/api/grocery/search", handleGrocerySearch);
...
async function handleDailyGame(req, res, next) {
  try {
...
app.get("/daily-game", handleDailyGame);
app.get("/api/daily-game", handleDailyGame);

async function handleDailyScoreGet(req, res) {
...
app.get("/daily-score", handleDailyScoreGet);
app.get("/api/daily-score", handleDailyScoreGet);

async function handleDailyScorePost(req, res, next) {
  try {
...
app.post("/daily-score", handleDailyScorePost);
app.post("/api/daily-score", handleDailyScorePost);
...
for (const method of ["get", "post"]) {
  app[method]("/rebuild-daily", async (req, res, next) => {
    try {
      const catalog = await rebuildDailyOnly();
      res.json({
        ok: true,
  ...
  app[method]("/api/rebuild-daily", async (req, res, next) => {
    try {
      const catalog = await rebuildDailyOnly();
      res.json({
        ok: true,
  ...
  app[method]("/full-update", async (req, res, next) => {
    try {
  ...
  app[method]("/api/full-update", async (req, res, next) => {
    try {
      const result = await fullUpdate();
      res.json(result);
    } catch (error) {
